...
(function ($) {
    $.fn.appoeForm = function (userOptions = {}) {
        ...
        $(document.body).on('submit', $(this), function (e) {
            e.preventDefault();
            let $form = $(e.target)
            if(!userOptions.publicKey){if ($form.hasAttr('data-key')){userOptions.publicKey = $form.attr('data-key');}}
            if(userOptions.publicKey){processFormPostRequest($form, userOptions.publicKey);}
            else {return false;}
        });
        ...
        return this.each(function (i, el) {
            if ($(el).length) {
                ...
                if(!options.publicKey){if ($(el).hasAttr('data-key')){options.publicKey = $(el).attr('data-key');}}
                if (!$(el).hasAttr('action')){$(el).attr('action', options.action);}
                if (!$(el).hasAttr('method')){$(el).attr('method', options.method);}
                if (!$(el).hasAttr('data-gaction')){$(el).attr('data-gaction', $(el).attr('data-ftype'));}
            }
        });
    }
})(jQuery);
