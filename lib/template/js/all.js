const $pageStatus = jQuery('#pageStatus');
const $libraryContainer = jQuery('#loadMediaLibrary');
let loadedMedia = false;

function showPageStatus(msg, options = {}) {
    ...
    if (options.delay) {
        $pageStatus.attr('data-delay', options.delay);
    }

    $pageStatus.find('div.toast-body').html(msg)
    $pageStatus.toast('show');
}

function hidePageStatus() {
    $pageStatus.toast('hide');
}
...
jQuery(document).ready(function ($) {
    ...
    $pageStatus.on('hidden.bs.toast', function () {
        $pageStatus.find('div.toast-body').html('');
    });
    ...
    $(document).on('dblclick', 'input.urlFile', function (event) {
        ...
        if (!loadedMedia) {
            showPageStatus(loaderHtml() + ' Chargement des média.');
