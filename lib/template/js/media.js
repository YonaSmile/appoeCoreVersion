...
function getMedia($container) {
    $($container).hide().load('/app/ajax/getMedia.php', function () {
        shortAccessLibrary();
...
$(document).ready(function () {
    ...
    if ($('#allMediaModalContainer').length) {
        ...
        $('form#mediaLibraryForm').on('submit', function (event) {
        ...
        $(window).on('click', function (e) {
            ...
            let $mediaDetails = $('#mediaDetails');
            if ($mediaDetails.has(e.target).length == 0 && !$mediaDetails.is(e.target)
                && $('#overlay').has(e.target).length == 0 && !$('#overlay').is(e.target)) {
                closeMediaDetails();
            }
        });
        ...
        $('#mediaGridPreferences').on('change', function () {
            ...
            $.post(
                ...
                function (data) {
                    if (data == 'true' || data === true) {
                        ...
                        let $img = $('img.seeOnOverlay', $container);
                        let $imgLibrary = $('div.card[data-file-id="' + id + '"] img');
                        let $copyLink = $('span.copyLinkOnClick', $container);
