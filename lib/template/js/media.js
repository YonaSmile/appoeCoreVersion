$(document).ready(function () {
    ...
    $(document.body).on('click', 'a.getMediaDetails', function (event) {
        ...
        let $btn = $(this);
        let $mediaDetailsBody = $('#mediaDetails div.offcanvas-body');
        let id = $btn.attr('data-file-id');

        openMediaDetails(id);
        $mediaDetailsBody.html('<div class="p-3">' + loaderHtml() + '</div>');
        $mediaDetailsBody.load('/app/ajax/getMediaDetails.php?fileId=' + id);
