...
{
    $html = '<div class="row"><div class="col-12 position-relative mb-3">
...
function checkConnectedUser(array $connectedUsers)
{
...
function checkFreePageConnectedUsers(array $connectedUsers)
{
...
function includePluginsFiles($forApp = false)
{
    ...
    if (is_array($plugins) && !empty($plugins)) {
        ...
        foreach ($plugins as $plugin) {
            ...
            if (file_exists($filePath) && !file_exists($plugin['setupPath']) && loadPluginForFilename($plugin['name'])) {
...
function includePluginsFilesForApp()
{
    ...
    if (is_array($plugins) && !empty($plugins)) {
        ...
        foreach ($plugins as $plugin) {
            ...
            if (file_exists($filePath) && !file_exists($plugin['setupPath'])) {
                foreach (getFilesFromDir($filePath, ['onlyExtension' => 'php']) as $file) {
                    $src = $filePath . DIRECTORY_SEPARATOR . $file;
                    include_once($src);
                }
