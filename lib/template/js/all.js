...
jQuery(window).on('load', function () {
    ...
    $(document.body).on('click', 'div.tab-header', function (e) {
        e.preventDefault();

        let tabHeader = $(this);
        let tab = tabHeader.closest('div.tab');
        if (!tabHeader.hasClass('tab-active')) {

            $('div.tab-header').removeClass('tab-active');
            tabHeader.addClass('tab-active');
            $('div.tab-content').slideUp();
            $('div#tab-content').html('').hide();

            if ($(document).width() > 770) {
                $('div#tab-content').show().html(getHtmlLoader());
                $('div#tab-content').html(tab.find('div.tab-content').html());
            } else {
                tab.find('div.tab-content').slideDown();
            }
        }
    });

    $(document.body).on('keyup', 'input#tab-search-input', function () {
        inputFilter('tab-search-input', 'div.tab');
    });
});