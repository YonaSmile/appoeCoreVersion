...
function closeMediaDetails() {
    ...
    var bsOffcanvas = new bootstrap.Offcanvas(mediaDetails);
    bsOffcanvas.hide();
    mediaDetails.setAttribute('data-file-id', '');
}

function openMediaDetails(fileId) {
    var mediaDetails = document.getElementById('mediaDetails');
    var bsOffcanvas = new bootstrap.Offcanvas(mediaDetails);
    bsOffcanvas.show();
    mediaDetails.setAttribute('data-file-id', fileId);
}
        ...
        var observer = new IntersectionObserver(
            function (entries) {
                if (entries[0].intersectionRatio === 0) {
                    document.querySelector(id).classList.add("isSticky");
                } else if (entries[0].intersectionRatio === 1) {
                    document.querySelector(id).classList.remove("isSticky");
                }
            },
            {threshold: [0, 1]}
        );
...
jQuery(window).on('load', function () {
    ...
    setTimeout(function () {
        $('div.admin-tab-header.admin-tab-first').trigger('click').removeClass('admin-tab-first');
        $('input#admin-tab-search-input').focus();
    }, 100);

    observeSticky('#admin-tab-search');
    ...
    $(document.body).on('click', 'div.admin-tab-header', function (e) {
        ...
        if (!tabHeader.hasClass('admin-tab-active')) {

            $('div.admin-tab-header').removeClass('admin-tab-active');
            tabHeader.addClass('admin-tab-active');
            $('div.admin-tab-content').slideUp();
            $('div#admin-tab-content').html('').hide();

            if ($(document).width() > 770) {
                $('div#admin-tab-content').show().html(getHtmlLoader());
                $('div#admin-tab-content').html(tab.find('div.admin-tab-content').html());
            } else {
                tab.find('div.admin-tab-content').slideDown(300, function () {
                    setTimeout(function () {
                        $('html,body').animate({
                            scrollTop: tab.offset().top - 102
                        }, 500);
                    }, 100);
                });
            }
