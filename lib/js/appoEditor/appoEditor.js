    ...
    if ($('textarea.appoeditor')) {
        ...
        $('.appoeditor').each(function (num, el) {
            ...
            var buttons = {
                ...
                buttonsMedia: [
                    '<button type="button" data-mark="a" data-id-modal="insertImg" class="openModalEditor noSelection btnEditor btn btn-secondary" title="Ajouter une image"><i class="far fa-image"></i></button>'
                ]
            };
...
$(document).ready(function () {
    ...
    if ($('textarea.appoeditor')) {
        ...
        $(document.body).on('click', 'button.openModalEditor', function () {
            ...
            selection = saveSelection();
            if (selection.toString().length > 0 || $(this).hasClass('noSelection')) {
