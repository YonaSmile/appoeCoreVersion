    ...
    $.fn.appoeForm = function (userOptions = {}) {
        return this.each(function () {
            ...
            if ($form.length) {
                ...
                if(!options.publicKey){if ($form.hasAttr('data-key')){options.publicKey = $form.attr('data-key');}}
                if (!$form.hasAttr('action')){$form.attr('action', options.action);}
                if (!$form.hasAttr('method')){$form.attr('method', options.method);}
                if (!$form.hasAttr('data-gaction')){$form.attr('data-gaction', $form.attr('data-ftype'));}

                $(document.body).on('submit', $form, function (e) {
                    e.preventDefault();

                    if(options.publicKey){processFormPostRequest($form, options.publicKey);}
                    else {return false;}
                });
            }
        });
