$(document).ready(function () {
    ...
    $(document.body).on('input', '#managePageInfo select#page', function () {
        $('#managePageInfo #textInfosZone').load('/app/ajax/plugin.php', {
            pageInfos: true,
            filename: $('select#page').val()
        }, function () {
            appoEditor();
        });
    });

    $(document.body).on('submit', 'form#managePageInfo', function (e) {
        e.preventDefault();

        var $form = $(this);
        var $btn = $('button[type="submit"]', $form);

        $.post('/app/ajax/plugin.php', $form.serialize(), function (data) {

            if (data === true || data == 'true') {
                $btn.html('Enregistré').addClass('btn-success').removeClass('btn-outline-primary');

                setTimeout(function () {
                    $btn.html('Enregistrer').addClass('btn-outline-primary').removeClass('btn-success');
                }, 1000);
            } else {
                $btn.html(data).addClass('btn-danger').removeClass('btn-outline-primary');

                setTimeout(function () {
                    $btn.html('Enregistrer').addClass('btn-outline-primary').removeClass('btn-danger');
                }, 1000);
            }
        });
    });
