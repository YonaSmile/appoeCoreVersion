...
function notification(msg, status = 'success') {
    ...
    if (!$('#alertFlashContainer').length) {
        $('body').append('<div id="alertFlashContainer" class="alertFlash"></div>');
    }

    let $container = $('#alertFlashContainer');
    let id = 'appoeNotif-' + Math.floor((Math.random() * 1000) + 1);
    let html = '<div id="' + id + '" class="alert alert-' + status + ' fade show" role="alert">' +
        '<img src="https://demo.aoe-communication.com/app/lib/template/images/appoe-logo-black-sm.png" width="25" style="vertical-align: initial;" alt="APPOE">' +
        '&nbsp; <strong class="text-dark">APPOE</strong><hr class="my-2">' + msg + '</div>';
...
jQuery(document).ready(function ($) {
    ...
    function setSidebarAndAreaContent() {
        ...
        if ($("#sidebar").hasClass('active')) {
            $contentArea.css('transform', 'translateX(0)')
            $contentArea.css('width', '100%');
        } else {
            if ($(window).width() > 770) {
                $contentArea.css('transform', 'translateX(240px)');
